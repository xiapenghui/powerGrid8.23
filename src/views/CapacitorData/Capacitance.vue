      ...
      <el-form ref="ruleForm" v-loading="editLoading" :model="ruleForm" :rules="rules" label-width="150px" class="demo-ruleForm">
        <div class="bigUpBox">
          <div class="boxLeft">
            ...
            <el-tooltip class="item" effect="dark" content="工频电压单位是kV，保留四位小数" placement="top-start">
              <el-form-item label="工频电压单位是kV，保留四位小数" prop="voltage">
...
import { drclList, drclDellte, drclEdit, allLogs } from '@/api/business'
...
export default {
  ...
  data() {
    return {
      productionUrl: this.GLOBAL.BASE_URL + '/demo/api/drq/dianrongqi11/import/file',
  ...
  methods: {
    ...
    getList() {
      ...
      drclList(this.pagination, this.listQuery).then(res => {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          drclEdit(this.ruleForm).then(res => {
            if (res.code === 200) {
              this.$message({
                type: 'success',
