<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" :label="$t('permission.unitCleanClass')" width="200" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          {{ scope.row.unitCleanClass }}
        </template>
      </el-table-column>

      <el-table-column align="center" :label="$t('permission.cleanLiness')" width="200" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          {{ scope.row.cleanLiness }}
        </template>
      </el-table-column>

      <el-table-column align="center" :label="$t('permission.unitCleanLiness')" width="200" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          {{ scope.row.unitCleanLiness }}
        </template>
      </el-table-column>
      ...
      <el-form ref="ruleForm" v-loading="editLoading" :model="ruleForm" :rules="rules" label-width="130px" class="demo-ruleForm">
        <div class="bigUpBox">
          <div class="boxLeft">
            ...
            <el-tooltip class="item" effect="dark" content="供应商数据唯一标识" placement="top-start">
              <el-form-item label="供应商数据唯一标识" prop="productModel"><el-input v-model="ruleForm.productModel" /></el-form-item>
            </el-tooltip>
            ...
            <el-tooltip class="item" effect="dark" content="洁净度（0.5μm），单位：个/m³" placement="top-start">
              <el-form-item label="洁净度（0.5μm），单位：个/m³" prop="cleanClass"><el-input v-model="ruleForm.cleanClass" /></el-form-item>
            </el-tooltip>
            <el-tooltip class="item" effect="dark" content="洁净度，仅数字，单位：个/升" placement="top-start">
              <el-form-item label="洁净度，仅数字，单位：个/升" prop="cleanLiness"><el-input v-model="ruleForm.cleanLiness" /></el-form-item>
            </el-tooltip>
          ...
          <div class="boxRight">
            ...
            <el-tooltip class="item" effect="dark" content="洁净度，仅数字，单位：个/升" placement="top-start">
              <el-form-item label="洁净度，仅数字，单位：个/升" prop="unitCleanClass"><el-input v-model="ruleForm.unitCleanClass" /></el-form-item>
            </el-tooltip>

            <el-tooltip class="item" effect="dark" content="洁净度，仅数字，单位：个/升" placement="top-start">
              <el-form-item label="洁净度，仅数字，单位：个/升" prop="unitCleanLiness"><el-input v-model="ruleForm.unitCleanLiness" /></el-form-item>
            </el-tooltip>
...
import { jjdList, jjdDellte, jjdEdit, allLogs } from '@/api/business'
...
export default {
  ...
  data() {
    return {
      productionUrl: this.GLOBAL.BASE_URL + '/linx/dianrongqi_6/import/file',
      ...
      rules: {
        ...
        cleanClass: [{ required: true, message: '请输入洁净度（0.5μm），单位：个/m³)', trigger: 'blur' }]
      }
    }
  },
  computed: {},
  ...
  methods: {
    ...
    getList() {
      ...
      jjdList(this.pagination, this.listQuery).then(res => {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          jjdEdit(this.ruleForm).then(res => {
            if (res.code === 200) {
              this.$message({
                type: 'success',
