<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" :label="$t('permission.productModelSBH')" width="220" :show-overflow-tooltip="true">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" :label="$t('permission.parDischangeUn')" width="150" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          {{ scope.row.parDischangeUn }}
        </template>
      </el-table-column>

      <el-table-column align="center" :label="$t('permission.parDischange')" width="150" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          {{ scope.row.parDischange }}
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    return {
      productionUrl: this.GLOBAL.BASE_URL + '/demo/api/drq/dianrongqi10/import/file',
      myHeaders: {
  ...
  methods: {
    ...
    getList() {
      ...
      jbfdList(this.pagination, this.listQuery).then(res => {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          jbfdEdit(this.ruleForm).then(res => {
            if (res.code === 200) {
              this.$message({
                type: 'success',
