<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column align="center" :label="$t('permission.productModelSBH')" width="180" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          {{ scope.row.productModel }}
        </template>
      </el-table-column>

      <el-table-column align="center" :label="$t('permission.cabWorkNo')" width="200" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          {{ scope.row.cabWorkNo }}
        </template>
      </el-table-column>

      <el-table-column align="center" :label="$t('permission.cabSN')" width="220" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          {{ scope.row.cabSN }}
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" :label="$t('permission.voltage')" width="150" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          {{ scope.row.voltage }}
        </template>
      </el-table-column>

      <el-table-column align="center" :label="$t('permission.durationUnS')" width="150" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          {{ scope.row.durationUn }}
        </template>
      </el-table-column>

      <el-table-column align="center" :label="$t('permission.durationS')" width="150" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          {{ scope.row.duration }}
        </template>
      </el-table-column>
    ...
    <el-dialog title="编辑信息" :close-on-click-modal="false" :visible.sync="dialogFormVisible">
      <el-form ref="ruleForm" v-loading="editLoading" :model="ruleForm" :rules="rules" label-width="130px" class="demo-ruleForm">
        <div class="bigUpBox">
          <div class="boxLeft">
            <el-form-item label="工厂名称" prop="salesOrg"><el-input v-model="ruleForm.salesOrg" :disabled="true" /></el-form-item>
            ...
            <el-form-item label="时长单位是s" prop="duration"><el-input v-model="ruleForm.duration" /></el-form-item>
          </div>
          <div class="boxRight">
            ...
            <el-tooltip class="item" effect="dark" content="额定电压值单位是kV，保留三位小数" placement="top-start">
              <el-form-item label="额定电压值单位是kV，保留三位小数" prop="voltageUn"><el-input v-model="ruleForm.voltageUn" /></el-form-item>
            </el-tooltip>
            <el-form-item label="额定时长单位是s" prop="durationUn"><el-input v-model="ruleForm.durationUn" /></el-form-item>
...
import { jjjlList, jjjlDellte, jjjlEdit, allLogs } from '@/api/business'
...
export default {
  ...
  data() {
    return {
      productionUrl: this.GLOBAL.BASE_URL + '/api/drq/dianrongqi_8/import/file',
      ...
      rules: {
        ...
        voltageUn: [{ required: true, message: '请输入额定电压值单位是kV，保留三位小数', trigger: 'blur' }],
        voltage: [{ required: true, message: '请输入电压值单位是kV，保留三位小数', trigger: 'blur' }]
      }
    }
  },
  computed: {},
  ...
  methods: {
    ...
    getList() {
      ...
      jjjlList(this.pagination, this.listQuery).then(res => {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          jjjlEdit(this.ruleForm).then(res => {
            if (res.code === 200) {
              this.$message({
                type: 'success',
