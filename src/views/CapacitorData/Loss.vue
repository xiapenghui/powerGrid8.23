...
export default {
  ...
  data() {
    return {
      productionUrl: this.GLOBAL.BASE_URL + '/demo/api/drq/dianrongqi12/import/file',
      myHeaders: {
      ...
      rules: {
        ...
        capacitanceUn: [{
          required: true,
          message: '请输入额定电容量单位是（μF），保留三位小数',
          trigger: 'blur'
        }],
        capacitance: [{
          required: true,
          message: '请输入电容量单位是（μF），保留三位小数',
          trigger: 'blur'
        }]
      }
    }
  ...
  methods: {
    ...
    getList() {
      ...
      drqshList(this.pagination, this.listQuery).then(res => {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          drqshEdit(this.ruleForm).then(res => {
            if (res.code === 200) {
              this.$message({
                type: 'success',
